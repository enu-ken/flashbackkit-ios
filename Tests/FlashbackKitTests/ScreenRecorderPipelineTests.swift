...
final class ScreenRecorderPipelineTests: XCTestCase {
    ...
    // MARK: - UI 由来オリエンテーション（フレーム内容のみ回転する端末のフォールバック）

    /// 添付なしフレームを流す途中で `noteInterfaceOrientation(.right)` を入れるケース
    /// （実機 iPhone 15 Pro：寸法も添付も変わらず内容だけ横倒しになる端末の挙動）。
    /// UI 由来の orientation 変化でリングがリセットされ（出力は後半ぶんのみ）、
    /// 出力トラックの preferredTransform が .right の正立化（+90°）になり表示サイズの W/H が入れ替わる。
    func testInterfaceOrientationChangeResetsRingAndAppliesUprightTransform() async throws {
        let writer = SegmentRingWriter(bufferSeconds: 3)   // segmentDuration=2, maxSegments=3
        let fps = 10
        let frameDuration = CMTime(value: 1, timescale: CMTimeScale(fps))

        let firstCount = 20                                // 2 秒（添付なし = UI フォールバック .up）
        let secondCount = 10                               // 1 秒（noteInterfaceOrientation(.right) 後）
        let w = 64, h = 48
        for i in 0..<(firstCount + secondCount) {
            // 後半の最初のフレーム投入の「直前」に UI 由来の向き変化を通知する。
            // noteInterfaceOrientation も ingest も同じシリアルキューに async 投入されるため、
            // この呼び出しは直後の append より前に直列実行される（明示同期は不要）。
            if i == firstCount { writer.noteInterfaceOrientation(.right) }
            let pts = CMTimeMultiply(frameDuration, multiplier: Int32(i))
            let pixelBuffer = try makePixelBuffer(width: w, height: h, frameIndex: i)
            // 添付は終始なし。リセットの引き金になるのは UI 由来の orientation 変化のみ。
            let sampleBuffer = try makeSampleBuffer(pixelBuffer: pixelBuffer, pts: pts, duration: frameDuration)
            writer.ingest(sampleBuffer, type: .video)
            try await Task.sleep(nanoseconds: 100_000_000)
        }
        ...
        // 1) リセット発火の証拠：尺は後半（≒1.0s）ぶんのみ。
        let seconds = CMTimeGetSeconds(try await asset.load(.duration))
        XCTAssertLessThan(seconds, 1.6, "UI 向き変化前のセグメントが残っている疑い（尺が長すぎ）: \(seconds)s")

        // 2) preferredTransform が .right の正立化（+90°）＝非 identity。
        let transform = try await track.load(.preferredTransform)
        XCTAssertNotEqual(transform, .identity, "UI 向き変化後も transform が identity のまま（正立化されていない）")

        // 3) 表示サイズの W/H が入れ替わる（64x48 → 48x64 相当）。
        let natural = try await track.load(.naturalSize)
        let displayed = natural.applying(transform)
        XCTAssertEqual(abs(displayed.width), CGFloat(h), accuracy: 0.5, "表示幅が H と入れ替わっていない")
        XCTAssertEqual(abs(displayed.height), CGFloat(w), accuracy: 0.5, "表示高さが W と入れ替わっていない")

        // 4) 期待の回転行列（+90°）。a≈0, b≈1, c≈-1, d≈0。
        XCTAssertEqual(transform.a, 0, accuracy: 0.001)
        XCTAssertEqual(transform.b, 1, accuracy: 0.001)
        XCTAssertEqual(transform.c, -1, accuracy: 0.001)
        XCTAssertEqual(transform.d, 0, accuracy: 0.001)
    ...
    /// 優先順位の確認：添付 .up 付きフレームを流した後で `noteInterfaceOrientation(.right)` を入れても
    /// 「添付が来る端末では添付が正」のため UI 由来は完全に無視される。
    /// → リセットは起きず（尺は全長）、transform は identity のまま。
    func testInterfaceOrientationIgnoredWhenAttachmentPresent() async throws {
        let writer = SegmentRingWriter(bufferSeconds: 3)   // segmentDuration=2, maxSegments=3
        let fps = 10
        let frameDuration = CMTime(value: 1, timescale: CMTimeScale(fps))

        let firstCount = 20                                // 2 秒（添付 .up）
        let secondCount = 10                               // 1 秒（noteInterfaceOrientation(.right) は無視される）
        let w = 48, h = 64
        for i in 0..<(firstCount + secondCount) {
            // 添付が一度でも来たら以後 UI 由来は無視されるはず（二重ソース競合の回避）。
            if i == firstCount { writer.noteInterfaceOrientation(.right) }
            let pts = CMTimeMultiply(frameDuration, multiplier: Int32(i))
            let pixelBuffer = try makePixelBuffer(width: w, height: h, frameIndex: i)
            // 終始 .up を添付（= sawOrientationAttachment が立つ）。
            let sampleBuffer = try makeSampleBuffer(pixelBuffer: pixelBuffer, pts: pts,
                                                    duration: frameDuration, orientation: .up)
            writer.ingest(sampleBuffer, type: .video)
            try await Task.sleep(nanoseconds: 100_000_000)
        ...
        // 1) UI 由来は無視 = リセットなし。尺は全長（3 秒ぶん程度）が残る。
        let seconds = CMTimeGetSeconds(try await asset.load(.duration))
        XCTAssertGreaterThan(seconds, 1.6, "UI 由来が無視されずリセットされた疑い（尺が短すぎ）: \(seconds)s")

        // 2) transform は identity（添付 .up が正・UI .right は不採用）。
        let transform = try await track.load(.preferredTransform)
        XCTAssertEqual(transform, .identity, "添付 .up なのに UI .right が採用され transform が非 identity")
    }

    /// 開始時スタンプの確認：ring 生成直後に `noteInterfaceOrientation(.left)` を入れてから
    /// 添付なしフレームを流す（= 横画面で録画を開始したケース）。
    /// 添付が来なくても export の transform が .left の正立化（−90°）になり W/H が入れ替わる。
    func testInterfaceOrientationStampedAtStartAppliesTransform() async throws {
        let writer = SegmentRingWriter(bufferSeconds: 3)   // segmentDuration=2, maxSegments=3
        // ring 生成直後のスタンプ（ScreenRecorder.startBuffering が行うのと同じ）。
        writer.noteInterfaceOrientation(.left)
        ...
        // 開始時スタンプが効いて全フレームが .left 起点 → 一度もリセットせず尺は全長。
        let seconds = CMTimeGetSeconds(try await asset.load(.duration))
        XCTAssertGreaterThan(seconds, 1.0, "尺が短すぎる（不要なリセットの疑い）: \(seconds)s")

        // preferredTransform が .left の正立化（−90°）。a≈0, b≈-1, c≈1, d≈0。
        let transform = try await track.load(.preferredTransform)
        XCTAssertNotEqual(transform, .identity, "開始時 .left スタンプが反映されず transform が identity")
        XCTAssertEqual(transform.a, 0, accuracy: 0.001)
        XCTAssertEqual(transform.b, -1, accuracy: 0.001)
        XCTAssertEqual(transform.c, 1, accuracy: 0.001)
        XCTAssertEqual(transform.d, 0, accuracy: 0.001)

        // 表示サイズの W/H が入れ替わる（64x48 → 48x64 相当）。
        let natural = try await track.load(.naturalSize)
        let displayed = natural.applying(transform)
        XCTAssertEqual(abs(displayed.width), CGFloat(h), accuracy: 0.5, "表示幅が H と入れ替わっていない")
        XCTAssertEqual(abs(displayed.height), CGFloat(w), accuracy: 0.5, "表示高さが W と入れ替わっていない")
